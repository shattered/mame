	...
	uint32_t *track, uint32_t *head, uint32_t *sector)
{
	...
	*head = (sector_index / sectors) % heads;
	...
	if (*track != di->cached_track || *head != di->cached_head)
	{
		...
		{
			ferr = floppy_get_indexed_sector_info(imgtool_floppy(image), 0, 0, i, NULL, NULL, &first_sector, NULL, NULL);
			if (ferr)
				return imgtool_floppy_error(ferr);

			if (first_sector > 0 && first_sector <= sectors)
				di->map[first_sector - 1] = i;
		}
...
static imgtoolerr_t rt11_probe_geometry(imgtool::image &image)
{
	...
	ferr = floppy_get_indexed_sector_info(imgtool_floppy(image), 0, 0, 0, NULL, NULL, &first_sector, &di->sector_size, NULL);
	if (ferr)
		return imgtool_floppy_error(ferr);
