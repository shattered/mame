...
//-------------------------------------------------
ie15_cpu_device::ie15_cpu_device(const machine_config &mconfig, const char *tag, device_t *owner, UINT32 clock)
...
UINT32 ie15_cpu_device::disasm_min_opcode_bytes() const
{
	return 1;
}
...
UINT32 ie15_cpu_device::disasm_max_opcode_bytes() const
{
	return 2;
}
...
offs_t ie15_cpu_device::disasm_disassemble(char *buffer, offs_t pc, const UINT8 *oprom, const UINT8 *opram, UINT32 options)
{
...
UINT32 ie15_cpu_device::execute_min_cycles() const
{
	return 1;
}
...
UINT32 ie15_cpu_device::execute_max_cycles() const
{
	return 1;
}
...
inline void ie15_cpu_device::illegal(UINT8 opcode)
{
...
inline UINT8 ie15_cpu_device::rop()
{
...
inline UINT8 ie15_cpu_device::arg()
{
	UINT8 retVal = m_direct->read_byte(m_PC.w.l);
	return retVal;
}

inline UINT8 ie15_cpu_device::get_reg_lo(UINT8 reg)
{
	UINT16 tmp = m_RF ? m_REGS[16 + reg] : m_REGS[reg];
	return tmp & 255;
}

inline UINT16 ie15_cpu_device::get_reg(UINT8 reg)
{
	return m_RF ? m_REGS[16 + reg] : m_REGS[reg];
}

inline void ie15_cpu_device::set_reg(UINT8 reg, UINT16 val)
{
	(m_RF ? m_REGS[16 + reg] : m_REGS[reg]) = val;

}

inline void ie15_cpu_device::update_flags(UINT8 val)
{
	m_ZF = (val == 0xff) ? 1 : 0;
}

inline UINT8 ie15_cpu_device::do_condition(UINT8 val)
{
	UINT8 v = (val >> 5) & 1;
	UINT8 cond = 0;
	switch((val>> 3) & 0x03) {
...
inline UINT16 ie15_cpu_device::get_addr(UINT8 val)
{
	UINT8 lo = arg();
