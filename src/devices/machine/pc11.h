...
#define MCFG_PC11_RXRDY_HANDLER(_write) \
	devcb = &pc11_device::set_rxrdy_callback(*device, DEVCB_##_write);

#define MCFG_PC11_TXRDY_HANDLER(_write) \
	devcb = &pc11_device::set_txrdy_callback(*device, DEVCB_##_write);
...
public:
	...
	template<class _Object> static devcb_base &set_rxrdy_callback(device_t &device, _Object object) { return downcast<pc11_device &>(device).m_write_rxrdy.set_callback(object); }
	template<class _Object> static devcb_base &set_txrdy_callback(device_t &device, _Object object) { return downcast<pc11_device &>(device).m_write_txrdy.set_callback(object); }
